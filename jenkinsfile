...
publishHTML([allowMissing: false, alwaysLinkToLastBuild: true, keepAll: true, reportDir: 'target/sonar/issues-report/', reportFiles: 'issues-report-light.html', reportName: 'Preview SonarQube OSDE', reportTitles: "Preview SonarQube ${JOB_BASE_NAME}"])
}
}
}
...
mvnUtils.sonarqube(datosMvn, "mvn ${maven_Debug} ${datosMvn.sonarqubePlugin} -Dsonar.login=${login} -Dsonar.password=${password} -Dsonar.jdbc.username=${jdbc_username} -Dsonar.jdbc.url='jdbc:jtds:sqlserver://SRVSQL323.osde.ar:1433/DBSonarQube;SelectMethod=Cursor'", debug)
}
vPipelineProgress = "${vPipelineProgress}OK\n"
...
dataFromMaven = mvnUtils.deploySNAPSHOT(datosMvn, "mvn ${maven_Debug} deploy -Dmaven.test.skip=true -DaltDeploymentRepository=aplicaciones-snapshot-jenkins::default::https://algo/content/repositories/aplicaciones-snapshot-jenkins/", debug)
vPipelineProgress = "${vPipelineProgress}:OK\n"
}
}
}
...
withCredentials([usernamePassword(credentialsId: 'svnjenkins', usernameVariable: 'svnUsername', passwordVariable: 'svnPassword')]) {
dataFromMaven = mvnUtils.release(datosMvn, "mvn ${maven_Debug} clean org.apache.maven.plugins:maven-release-plugin:2.5.3:prepare -DautoVersionSubmodules=true -DreleaseVersion=${datosMvn.nuevaVersion} -Dusername=${svnUsername} -Dpassword=${svnPassword} ", "mvn ${maven_Debug} -B org.apache.maven.plugins:maven-release-plugin:2.5.3:perform -Darguments='-Dmaven.javadoc.skip=true -DaltDeploymentRepository=aplicaciones-releases-jenkins::default::https://repoversiones.osde.ar:8443/content/repositories/aplicaciones-releases-jenkins/'", true)
}
vPipelineProgress = "${vPipelineProgress}:OK\n"
}
}
